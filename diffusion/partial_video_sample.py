...
def main():
    ...
    if args.cond_generation:
        ...
        guide_num = ""
        for i in args.guide_frames:
            if i == ",":
                guide_frames.append(int(guide_num))
                guide_num = ""
            else:
                guide_num = guide_num + i
                
        ref_frames = list(i for i in range(args.seq_len) if i not in cond_frames)
        logger.log(f"cond_frames: {cond_frames}")
        logger.log(f"guide_frames: {guide_frames}")
    ...
    cond_kwargs["guide_frames"] = guide_frames
    ...
    while len(all_videos) * args.batch_size < args.num_samples:
        ...
        if args.cond_generation:
            video, _ = next(data)
            cond_kwargs["cond_img"] = video[:,:,guide_frames].to(dist_util.dev()) 
